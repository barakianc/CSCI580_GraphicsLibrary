...
int Application5::Initialize()
{
	...
	GzToken		nameListAAOffset[2];
	GzPointer	valueListAAOffset[2];
	...
	//init a display and a renderer for each of the AA filter offsets
...
*/
	...
	*/
		...
		/*pass in the AA offsets*/
		for(int i = 0; i < AAKERNEL_SIZE; i++){
			nameListAAOffset[0] = GZ_AASHIFTX;
			valueListAAOffset[0] = (GzPointer)&AAFilter[i][0];
			nameListAAOffset[1] = GZ_AASHIFTY;
			valueListAAOffset[1] = (GzPointer)&AAFilter[i][1];
			status |= GzPutAttribute(m_pRender[i], 2, nameListAAOffset, valueListAAOffset);
...
int Application5::Render() 
{
	...
	*/ 
	...
	for(int i = 0; i < (m_nWidth*m_nHeight); i ++){
		...
		m_pDisplay[0]->fbuf[i].red *= AAFilter[0][2];
		m_pDisplay[0]->fbuf[i].green *= AAFilter[0][2];
		m_pDisplay[0]->fbuf[i].blue *= AAFilter[0][2];
		
		for(int j = 1; j < AAKERNEL_SIZE; j++){
			m_pDisplay[0]->fbuf[i].red += m_pDisplay[j]->fbuf[i].red * AAFilter[j][2];
			m_pDisplay[0]->fbuf[i].green += m_pDisplay[j]->fbuf[i].green * AAFilter[j][2];
			m_pDisplay[0]->fbuf[i].blue += m_pDisplay[j]->fbuf[i].blue * AAFilter[j][2];
		}
